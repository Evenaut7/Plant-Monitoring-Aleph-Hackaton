...
class ChartManager:
    ...
    @staticmethod
    def plot_pests_incidence(observations_collection):
        pests_counts = Counter(obs.pests for obs in observations_collection)
        labels = list(pests_counts.keys())
        counts = list(pests_counts.values())

        plt.figure(figsize=(10, 6))
        plt.bar(labels, counts, color='#e57373')
        plt.title("Active Pests Incidence")
